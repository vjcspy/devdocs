...
- Test runner: `yarn test` (or `yarn unit-test`) using mocha + ts-node with nyc coverage gates (test: statements 88%, functions 90%, branches 60%, lines 87; unit-test stricter: 90/90/70/90).  
- Coverage reports: nyc HTML plus `nyc check-coverage`. ESLint runs after tests.  
- Test suites:  
  - Controllers: Ecare and ZSP notification integration tests (`test/controller/*`).  
  - Services: Ecare API/auth/mapping/team services and ZSP mapping (`test/ecareService/*`, `test/zspService/*`).  
  - Repositories: MySQL integration tests with fixtures (`test/repository/*`).  
  - Utilities/fixtures under `test/utils` and `test/fixtures` for payloads and DB setup.  
- Helper libs: `tiny-testing`, `nock`, `supertest`, `ts-mockito`, and fixtures for external API stubbing.

## Operational Notes

- Environment selection via `ENVIRONMENT` env var (`dev`, `academy`, `production`) controls email config and relation-ID lists for pickup vs standard flows.  
- Requires Kong-authenticated integration users; all external calls retry once after token refresh.  
- Error handling distinguishes client errors vs 500s; Slack is notified for Ecare errors, and generic Dutch user messages are returned to callers.
